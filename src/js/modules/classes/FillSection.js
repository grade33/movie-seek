...
import { apiData, cssSelData, htmlData, sectionData } from '../data.js';
import { getRequestData } from '../helpFunctions.js';
...
export class FillSection {
  constructor(section) {
    ...
    if (this.sectionSettings.type === 'catalog') {
      this.handlerSelectNewPage();
      this.#handlerSelectGenre();
    }
  }
  ...
  handlerSelectNewPage() {
    this.paginationWrapEl.addEventListener('click', async (e) => {
      const prevBtn = e.target.contains(cssSelData.paginationBtnPrev);
      const nextBtn = e.target.contains(cssSelData.paginationBtnNext);
      const paginationBtnEl = e.target.contains(cssSelData.paginationBtn);
      if (paginationBtnEl) {
  ...
  #handlerSelectGenre() {
    this.genreWrapEl.addEventListener('click', async (e) => {
      ...
      const genreId = this.sectionSettings.request.genreId;
      const page = this.sectionSettings.request.page;
      console.log(this.sectionSettings);
      this.requestData = await getRequestData({ queryType, querySetting, genreId, page });
      console.log(this.requestData);
  ...
  #setRatingColor(ratingEl) {
    if (+ratingEl.innerHTML === 0) ratingEl.remove();
    else if (+ratingEl.innerHTML >= 7) ratingEl.style.background = '#007b00';
    else if (+ratingEl.innerHTML < 5) ratingEl.style.background = '#ff0b0b';
  }
